...
class AgentIQAssistant:
    def __init__(self, analytics_engine, graph_engine: FraudGraphEngine):
        ...
        self.agentic_engine = AgenticQueryEngine()
        
    def set_custom_api_config(self, api_key: Optional[str] = None, provider: str = "gemini", model_name: Optional[str] = None):
        """Passes custom user API key and model selection down to the query engine."""
        self.agentic_engine.set_custom_api_config(api_key=api_key, provider=provider, model_name=model_name)
        
    def answer_query(self, query: str, history: Optional[List[Dict[str, str]]] = None) -> Dict[str, Any]:
        """
        Processes natural language queries dynamically using AgenticQueryEngine.
        Supports any arbitrary dataset-aware question, data calculation, and dynamic visualization.
        """
        q = query.strip().lower()
        
        # Check for Graph Ring / Mule Account specialized engine queries
        if any(w in q for w in ['fraud ring', 'mule', 'syndicate', 'shared account', 'clusters', 'detect mule accounts']):
            ...
            res_df = cluster_df[['cluster_id', 'node_count', 'user_count', 'merchant_count', 'total_disputes', 'total_disputed_amount', 'syndicate_risk_score', 'risk_tier']].head(10) if not cluster_df.empty else pd.DataFrame()
            return {
                ...
                "metadata": {"intent": "graph_analysis"}
            }
            
        # Delegate to Agentic Query Engine for dynamic NLP dataset analytics
        res = self.agentic_engine.answer_query(query, history=history)
        
        # Ensure backward compatibility aliases for legacy UI parameters
        vis = res.get("visualization") or {}
        res["text"] = res.get("answer", "")
        res["chart_type"] = vis.get("type", "table")
        res["x"] = vis.get("xField", "")
        res["y"] = vis.get("yField", "")
        res["y_label"] = vis.get("yLabel", "")
